    ...
    for entry in pleco_entries:
        ...
        for head,info in meaning.items():
            if head in header_dict['ANC']['opt']:
                ...
                # in rare cases I might have forgottwen to delete stuff
                removerest=extracted_links[-1].split(' ',1)
                lastentry=removerest[:1]
                c=", ".join([simpl, trad, pron])
                if len(removerest)>1: print(f'WARNING: character ({c}) has unrelevant text at the end. Check original file.')
                # create dict with all relevant infos
                clean_entry['ANC']=ancient_forms
                clean_entry['LIN']=extracted_links[:-1]+lastentry
            else:
