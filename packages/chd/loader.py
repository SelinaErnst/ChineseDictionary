...
header_dict={
    ...
    'MNE':{
        'pr':'MNEMONICS',
        'opt':['MNEOMICS','MNE','MNEMONICS']},
    'USE':{
        'pr':'MEANING AS COMPONENT',
        'opt':['COMPONENT USAGE','USAGE','USE AS COMPONENT','MEANING AS COMPONENT']},
    'ORG':{
        'pr':'ORIGINS',
        'opt':['ORG','ORIGINS']},
    'ANC':{
...
def read_plecotxt(file:str) -> list:
        ...
        for row in tsv_reader:
            ...
            for text in re.split(f'{cs}',char_meaning):
                ...
                if text!='':
                    ...
                    if 'http' in text:
                        split = splitting(text,"http")
                        text = split[0]
                        links = split[1:]
                        char_entry += [['LINKS',' '.join(links)]]
    ...
    for entry in pleco_entries:
        ...
        for head,info in meaning.items():
            ...
            if head in header_dict['LIN']['opt']:
                elements = info.split(' ')
                # in rare cases I might have forgottwen to delete stuff (at the very end)
                rest=elements[-1].split(' ',1)
                last_link=rest[:1]
                extracted_links=elements[:-1]+last_link
